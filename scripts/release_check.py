...
def valid_release_ref(
    branch: str,
    tags_at_head: set[str],
    ref_type: str,
    ref_name: str,
) -> bool:
    if branch == "main":
    ...
    return ref_type == "tag" and ref_name == expected_tag and expected_tag in tags_at_head


def check_release_ref() -> None:
    branch = capture(["git", "branch", "--show-current"]).strip()
    tags_at_head = set(capture(["git", "tag", "--points-at", "HEAD"]).splitlines())
    check(
        valid_release_ref(
            branch,
            tags_at_head,
        ...
        f"release must be on main or the exact v{VERSION} GitHub tag",
    )
...
def main() -> int:
    ...
    verify_sample_manifest(ROOT / "examples" / "synthetic_export")
    ...
    print(
        "Release check passed: source, tests, artifacts, sample integrity, privacy, "
        "and author identity."
    )
    return 0
